...
class MockAPI {
  ...
  // Auth endpoints
  async register(data: { email; password; first_name; last_name; phone_number?; otp_code? }) {
  ...
  async getCheckoutTimer(reservationId) {
    await this.delay(200);
    const expiresAt = new Date();
    expiresAt.setMinutes(expiresAt.getMinutes() + 12);
  ...
  async rejectOrganizer(organizerId, reason?) {
    await this.delay(500);
