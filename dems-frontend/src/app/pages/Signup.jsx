...
export function Signup() {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      return true;
    } catch (error) { toast.error(error.message || 'Failed to send OTP'); return false; }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/discover');
    } catch (error) { setOtpError('Invalid OTP code. Please try again.'); toast.error(error.message || 'Failed to verify OTP'); }
    finally { setLoading(false); }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} disabled={otpSent} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
            ...
            {!otpSent && (
              <button type="button" onClick={handleSendOTP} disabled={loading} className="w-full px-4 py-3 rounded-lg font-semibold transition-colors disabled:opacity-50 mt-6 flex items-center justify-center gap-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 hover:bg-gray-800 dark:hover:bg-gray-100">
              ...
              <>
                ...
                <div>
                  ...
                  <input type="text" id="otp" value={otp} onChange={e => { setOtp(e.target.value.replace(/\D/g, '').slice(0, 6)); setOtpError(''); }}
                    required maxLength={6} autoFocus className={`${inputClass} text-center text-2xl tracking-widest font-semibold`} placeholder="000000" />
                  {otpError && <p className="text-sm text-red-600 mt-2">⚠ {otpError}</p>}
                  <div className="mt-2 flex items-center justify-between">
                    <button type="button" onClick={() => { setOtpSent(false); setOtp(''); setOtpError(''); }} className="text-sm underline text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">Change email</button>
                    <button type="button" onClick={handleSendOTP} disabled={loading} className="text-sm font-semibold hover:underline disabled:opacity-50 text-gray-900 dark:text-white">Resend OTP</button>
                  </div>
                </div>
                <button type="submit" disabled={loading || otp.length !== 6} className="w-full px-4 py-3 rounded-lg font-semibold transition-colors disabled:opacity-50 mt-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 hover:bg-gray-800 dark:hover:bg-gray-100">
