...
export function AdminEvents() {
  ...
  const filtered = search
    ? events.filter(e => e.title.toLowerCase().includes(search.toLowerCase()) || e.organizer_profile?.organization_name?.toLowerCase().includes(search.toLowerCase()))
    : events;

  const totalPages = Math.ceil(total / limit);

  const statusBadge = (status) => {
    const colors = { published: "bg-green-100 text-green-800", draft: "bg-gray-100 text-gray-800", cancelled: "bg-red-100 text-red-800", completed: "bg-blue-100 text-blue-800" };
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-4 mb-6 flex flex-wrap gap-4 items-center border border-transparent dark:border-gray-800">
          <div className="relative flex-1 min-w-48">
            ...
            <input type="text" placeholder="Search by title or organizer..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900" />
          </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    <tr>
                      {["Event", "Category", "Status", "Flags", "Created", "Actions"].map(h => (
                        <th key={h} className="text-left px-6 py-3 font-semibold text-gray-700 dark:text-gray-300">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {filtered.map(event => (
                      <tr key={event.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        ...
                        <td className="px-6 py-4"><span className={"inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium " + statusBadge(event.status)}>{event.status}</span></td>
                        <td className="px-6 py-4">
                          <button onClick={() => handleUpdate(event.id, { is_featured: !event.is_featured })} title="Toggle featured" className={"p-1 rounded " + (event.is_featured ? "text-yellow-500" : "text-gray-300 hover:text-yellow-400")}>
                            <Star className="size-4" fill={event.is_featured ? "currentColor" : "none"} />
                          </button>
                        </td>
                        <td className="px-6 py-4 text-gray-500 dark:text-gray-400">{new Date(event.created_at).toLocaleDateString()}</td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <Link to={"/events/" + event.id} className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"><Eye className="size-3" /> View</Link>
                            {event.status === "draft" && <button onClick={() => handleUpdate(event.id, { status: "published" })} className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-green-700 bg-green-50 hover:bg-green-100 rounded-lg transition-colors"><CheckCircle className="size-3" /> Publish</button>}
                            {event.status === "published" && <button onClick={() => handleUpdate(event.id, { status: "cancelled" })} className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-red-700 bg-red-50 hover:bg-red-100 rounded-lg transition-colors"><XCircle className="size-3" /> Cancel</button>}
                          </div>
                        </td>
                      </tr>
                    ))}
