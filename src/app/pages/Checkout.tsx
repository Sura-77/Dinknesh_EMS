...
export function Checkout() {
  ...
  // Chapa redirects back with ?tx_ref=... after payment
  ...
  const handlePayWithChapa = async () => {
    ...
    try {
      ...
      if (checkoutData.payment_url && checkoutData.payment_url !== '#') {
        ...
        toast.error('Payment gateway not configured. Please set CHAPA_SECRET_KEY in backend .env');
      }
    } catch (err: any) {
                ...
                {reservations.map((reservation) => (
                  <div key={reservation.id} className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="text-sm mt-0.5 text-gray-500 dark:text-gray-400">{reservation.ticket_type?.tier_name} × {reservation.quantity}</div>
                    </div>
          ...
          <div className="lg:col-span-1">
            <div className="rounded-xl shadow-sm p-6 sticky top-24 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800">
              ...
              <button onClick={handlePayWithChapa} disabled={processing}
                ...
                {processing ? <><div className="animate-spin size-4 border-2 border-white dark:border-gray-900 border-t-transparent rounded-full" />Redirecting...</> : <><ExternalLink className="size-4" />Pay with Chapa</>}
              </button>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="space-y-4">
                {reservations.map((reservation) => (
                  <div key={reservation.id} className="flex justify-between items-start">
                    <div>
                      <div className="font-semibold text-gray-900">{reservation.event?.title}</div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 flex gap-3">
              ...
              <div className="text-sm text-blue-800">
                You'll be redirected to Chapa's secure payment page to complete your purchase.
                Supported methods: Telebirr, CBE Birr, bank transfer, and cards.
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="pt-3 border-t border-gray-200 flex items-center justify-between">
                  ...
                  <span className="text-2xl font-bold text-gray-900">{totals.total.toFixed(2)} ETB</span>
                </div>
              </div>
              ...
              <button
                ...
                disabled={processing}
                className="w-full px-4 py-3 bg-gray-900 text-white rounded-lg font-semibold hover:bg-gray-800 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {processing ? (
                  <>
