          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div>
              <div className="inline-block px-3 py-1 rounded-full text-sm font-medium mb-4 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">
                {(event as any).category?.name || event.category_name}
              </div>
              ...
              <div className="space-y-4 mb-8">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400">{formatTime(event.schedule.start_datetime)} - {formatTime(event.schedule.end_datetime)}</div>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400">{event.organizer_name || (event as any).organizer_profile?.organization_name}</div>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-4">
                ...
                <button onClick={handleSaveEvent}
                  className={`px-6 py-3 border-2 rounded-lg font-semibold transition-colors flex items-center gap-2 ${isSaved ? 'bg-gray-900 dark:bg-white border-gray-900 dark:border-white text-white dark:text-gray-900' : 'bg-white dark:bg-transparent border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white hover:border-gray-400 dark:hover:border-gray-500'}`}>
                ...
                <div key={review.id} className="border-b pb-6 last:border-0 border-gray-200 dark:border-gray-800">
                  <div className="flex items-center gap-3 mb-2">
                    <div className="size-10 rounded-full flex items-center justify-center bg-gray-200 dark:bg-gray-700">
                      <span className="font-semibold text-gray-700 dark:text-gray-300">{(review.user_name || (review as any).user?.full_name || 'A')[0]}</span>
                    </div>
                    <div>
                      <div className="font-semibold text-gray-900 dark:text-white">{review.user_name || (review as any).user?.full_name}</div>
