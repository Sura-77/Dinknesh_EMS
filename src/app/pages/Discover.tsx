...
export function Discover() {
  ...
  const [nearMeStatus, setNearMeStatus] = useState<'idle' | 'requesting' | 'active' | 'denied'>('idle');
  ...
  useEffect(() => {
    ...
    if (userLocation && activeTab === 'near-me') {
      const nearby = allEvents.filter(e => {
        const lat = (e as any).venue?.latitude;
        const lon = (e as any).venue?.longitude;
  ...
  const applyFilters = useCallback(() => {
    ...
    if (q) {
      result = result.filter(e =>
        ...
        (e.category_name || (e as any).category?.name || '').toLowerCase().includes(q) ||
        ((e as any).tag_map || []).some((tm: any) => tm.tag?.name?.toLowerCase().includes(q))
      );
    }
    if (filters.category) {
      result = result.filter(e => (e as any).category?.id === filters.category || e.category_id === filters.category);
    }
    if (filters.location_type) {
      result = result.filter(e => (e as any).venue?.location_type === filters.location_type);
    }
    if (filters.city) {
      result = result.filter(e => (e as any).venue?.city?.toLowerCase().includes(filters.city.toLowerCase()));
    }
    if (filters.min_price) {
      result = result.filter(e => {
        const tickets = (e as any).ticket_types || [];
        return tickets.some((t: any) => t.price >= Number(filters.min_price));
      });
    }
    if (filters.max_price) {
      result = result.filter(e => {
        const tickets = (e as any).ticket_types || [];
        return tickets.some((t: any) => t.price <= Number(filters.max_price));
      });
    }
    ...
    if (filters.tag) {
      result = result.filter(e =>
        ((e as any).tag_map || []).some((tm: any) => tm.tag?.slug === filters.tag || tm.tag?.name?.toLowerCase() === filters.tag.toLowerCase())
      );
    }
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex gap-8">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Trending — only when no filters active and on all tab */}
            ...
            {/* Results header — only on all tab */}
            {activeTab === 'all' && (
            <>
            <div className="flex items-center justify-between mb-5">
              ...
              {isFiltering && (
                <div className="flex flex-wrap gap-2">
                  {filters.q && <span className={chipClass(true)}>"{filters.q}" <button onClick={() => setFilter('q', '')} className="ml-1">×</button></span>}
                  {filters.tag && <span className={chipClass(true)}>#{filters.tag} <button onClick={() => setFilter('tag', '')} className="ml-1">×</button></span>}
                  {filters.city && <span className={chipClass(true)}>{filters.city} <button onClick={() => setFilter('city', '')} className="ml-1">×</button></span>}
                  {filters.location_type && <span className={chipClass(true)}>{filters.location_type} <button onClick={() => setFilter('location_type', '')} className="ml-1">×</button></span>}
                  {filters.min_rating && <span className={chipClass(true)}>{filters.min_rating}+ ★ <button onClick={() => setFilter('min_rating', '')} className="ml-1">×</button></span>}
                </div>
              )}
            </div>
                ...
                <div className="rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800 shadow-sm">
                  ...
                  <div className="p-4 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800">
                    <p className="text-sm text-gray-500 dark:text-gray-400 text-center">
                      Showing events in Addis Ababa · {filteredEvents.filter(e => (e as any).venue?.city === 'Addis Ababa').length} events in this area
                    </p>
