...
export function CreateEvent() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Build ticket types array â€” only include tiers that have both price and capacity
    const ticket_types = [];
    if (formData.normal_price && formData.normal_capacity) {
      ticket_types.push({
        tier_name: 'Normal',
                ...
                <div className="border border-gray-300 rounded-lg p-2 focus-within:ring-2 focus-within:ring-gray-900 min-h-[44px] flex flex-wrap gap-2">
                  {tags.map(tag => (
                    <span key={tag} className="inline-flex items-center gap-1 px-2 py-1 bg-gray-900 text-white text-xs rounded-full">
                      #{tag}
                      <button type="button" onClick={() => removeTag(tag)} className="hover:text-gray-300">Ã—</button>
                    </span>
                  ))}
                  <input
                    type="text"
            ...
            {bannerMode === 'upload' ? (
              !bannerPreview ? (
                ...
                <div className="relative">
                  ...
                  {imageUrl && <div className="absolute bottom-3 left-3 px-2 py-1 bg-green-600 text-white text-xs rounded-lg">âœ“ Uploaded</div>}
                </div>
              )
            ) : (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="number" name={priceKey} required={required} min="0"
                        value={formData[priceKey]} onChange={handleChange}
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent"
                        placeholder="500" />
                    </div>
                    <div>
                      ...
                      <input type="number" name={capKey} required={required} min="1"
                        value={formData[capKey]} onChange={handleChange}
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent"
                        placeholder="500" />
                    </div>
                  </div>
