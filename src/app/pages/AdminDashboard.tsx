                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-600 dark:text-gray-400">
                          ...
                          {org.website_url && <div className="md:col-span-2"><span className="font-medium">Website:</span>{' '}
                            <a href={org.website_url} target="_blank" rel="noopener noreferrer" className="text-gray-900 dark:text-white hover:underline">{org.website_url}</a>
                          </div>}
                          <div className="md:col-span-2"><span className="font-medium">Submitted:</span> {daysAgo(org.created_at)}</div>
                        </div>
                      </div>
                      <div className="flex flex-col gap-2">
                        <button onClick={() => handleApprove(org.id)} className="flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 text-sm font-medium">
                          <CheckCircle className="size-4" /> Approve
                        </button>
                        <button onClick={() => handleReject(org.id)} className="flex items-center gap-2 px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 text-sm font-medium">
              ...
              <div key={event.id} className="px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                    ...
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${event.status === 'published' ? 'bg-green-100 text-green-800' : event.status === 'draft' ? 'bg-gray-100 text-gray-800' : 'bg-red-100 text-red-800'}`}>
                      {event.status}
                    </span>
                  </p>
                </div>
