...
router.get('/organizers/pending', authenticate, requireRole('admin'), async (_req, res: Response) => {
...
router.patch('/organizers/:id/approve', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
...
// GET /api/admin/users
router.get('/users', authenticate, requireRole('admin'), async (req, res: Response) => {
...
router.patch('/users/:id', authenticate, requireRole('admin'), async (req, res: Response) => {
...
router.get('/events', authenticate, requireRole('admin'), async (req, res: Response) => {
...
router.patch('/events/:id', authenticate, requireRole('admin'), async (req, res: Response) => {
  const event = await prisma.event.update({ where: { id: req.params.id }, data: req.body });
  res.json(event);
});

// GET /api/admin/categories
router.get('/categories', authenticate, requireRole('admin'), async (_req, res: Response) => {
  const categories = await prisma.eventCategory.findMany({ orderBy: { name: 'asc' } });
  res.json(categories);
});

// POST /api/admin/categories
router.post('/categories', authenticate, requireRole('admin'), async (req, res: Response) => {
...
router.patch('/categories/:id', authenticate, requireRole('admin'), async (req, res: Response) => {
