...
import { setReservationTimer, cancelReservationTimer } from '../services/timer';
...
router.post('/reservations', authenticate, async (req: AuthRequest, res: Response) => {
...
router.get('/reservations/list', authenticate, async (req: AuthRequest, res: Response) => {
...
router.patch('/reservations/:id', authenticate, async (req: AuthRequest, res: Response) => {
...
router.delete('/reservations/:id', authenticate, async (req: AuthRequest, res: Response) => {
  const reservation = await prisma.ticketReservation.findFirst({
    where: { id: req.params.id, user_id: req.user!.id, status: 'active' },
  });
  if (!reservation) { res.status(404).json({ error: 'Reservation not found' }); return; }
